...
각 Discussion 글의 본문 최상단에 같은 이름과 번호를 한 줄만 적으면 자동으로 하나의
시리즈로 묶입니다.
...
프롤로그 글이 필요하면 `# 시리즈 0`처럼 0번부터 시작해도 됩니다.

번호 표기는 아래 형식 중 하나를 선택할 수 있습니다. 굵은 표제를 사용할 때는 양쪽을
`**`로 감싸 주세요.
...
번호를 제외한 이름은 모든 글에서 같아야 합니다. 예를 들어 `ThorVG 렌더링 흐름`과
`ThorVG Rendering 흐름`은 서로 다른 시리즈로 인식됩니다.

본문 최상단의 표제는 시리즈를 묶는 용도입니다. 실제 시리즈 목록에는 각 Discussion의
글 제목이 표시되므로 표제와 글 제목은 달라도 됩니다.
...
본문 첫 제목의 시리즈 이름 사이에 `>`를 넣으면 트리로 묶입니다.
Discussion에서도 같은 규칙을 사용합니다. 아래는 각각 다른 글에 넣는 첫 제목입니다.
...
`Core2026` 아래에 `Issue`와 `Engine` 그룹이 생기고,
`Engine` 아래에는 개요 글과 `CPU Engine` 하위 그룹이, 그 아래에는 `Pipeline` 그룹이 표시됩니다.
그룹을 위한 별도 글이나 설정 파일은 필요 없습니다.

- 기존 `# Core2026 - 1` 표기의 글은 Core2026 바로 아래에 그대로 표시됩니다.
- 마지막 숫자로 글 순서를 정합니다. 그룹은 그 안의 가장 작은 글 번호를 기준으로 배치됩니다.
- 하위 그룹에서 번호를 다시 시작할 수도 있습니다. 상위 순서까지 유지하려면 기존 번호를 사용하세요.
- 그룹명에는 `/`를 쓸 수 있습니다. `>`만 계층 구분자로 사용합니다.
- 사이트에서 현재 글의 상위 그룹은 자동으로 펼쳐집니다. 본문의 분류용 첫 제목은 숨겨집니다.

Discussion 자체에 사이트의 접이식 메뉴가 생기는 것은 아니며, 동기화된 사이트에서 트리로 표시됩니다.
