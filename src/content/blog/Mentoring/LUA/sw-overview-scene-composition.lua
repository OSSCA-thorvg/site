...
-- @contain: direct_nested_parent=direct_nested_box:12
...
-- @contain: group_nested_parent=group_nested_box:12
...
local function label(g,id,text,x,y,size)
    return g:text{id=id,text=text,point=p(x,y),font="Pretendard",role="text",size=size or 30,
        align={0,0.5},fill="#344158",layer=60}
end
local function box(g,id,x,y,w,h,fill)
    return g:rectangle{id=id,center=p(x+w/2,y+h/2),size={w,h},corner=10,
        fill=fill or "#ffffff",stroke="#b5bec9",width=1.8,layer=10}
end
local model={
...
for _,phase in ipairs({"direct","group"}) do
    ...
    box(root,phase.."_root_box",60,170+dy,760,710,"#fafbfd")
    label(root,phase.."_root_label","root Scene · parent",95,222+dy,32)
    label(scene,phase.."_title",phase=="direct" and "Direct · opacity(255)" or "Group Opacity · opacity("..groupOpacity..")",60,90+dy,36)
    local childCircle=root:group{id=phase.."_circle_instance"}
    box(childCircle,phase.."_circle_box",95,330+dy,240,275)
    label(childCircle,phase.."_circle_label","child 1 · Shape",111,375+dy,25)
    local nested=root:group{id=phase.."_nested_instance"}
    box(nested,phase.."_nested_box",385,330+dy,395,440,"#f8f5fc")
    label(nested,phase.."_nested_label","child 2 · Scene",410,375+dy,27)
    label(nested,phase.."_nested_parent","parent",410,430+dy,24)
    label(nested,phase.."_triangle_child","child 1",440,630+dy,25)
    label(nested,phase.."_square_child","child 2",625,630+dy,25)
    ...
    label(scene,phase.."_buffer_label","target buffer",lane.target.x-210,330+dy,32)
end
scene:line{id="comparison_divider",from=p(60,1000),to=p(2140,1000),stroke="#b5bec9",width=1.8,layer=10}
...
label(intermediate,"intermediate_label","offscreen RGBA",scratch.x-210,1330,32)
...
local compositeArrow=scene:arrow{id="bitmap_composite_arrow",from=p(1485,target.y),to=p(1595,target.y),stroke="#8192a8",width=2.5,tip=13,layer=15}
local compositeLabel=label(scene,"composite_label","composite × "..groupOpacity.."/255",1270,1895,30)
...
scene:stroke(lanes.group.border,"#7296bd",.3)
scene:wait(2.5)
return scene
