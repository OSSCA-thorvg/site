...
const binary=process.argv[2]??'/tmp/thorvg-mask-methods-native';
...
await fs.writeFile('public/tmath/mask-methods/native-trace.mjs',`// Generated from the unmodified native CPU renderer.\nexport const commit='${commit}';\nexport default ${JSON.stringify(baseline)};\nexport const variant=${JSON.stringify(variant)};\n`);
console.log(Object.fromEntries(Object.entries(baseline.cases).map(([k,v])=>[k,v.buffers.map(b=>[b.channels,b.method])])));
