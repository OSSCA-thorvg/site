...
import {trace, rgba, compositeTerms, replaySpans} from './native-evidence.mjs';
...
export function buildPipeline(id) {
  ...
  const width = 960;
  const height = id === 'output' ? 750 : 540;
  ...
  function route(points, color = C.line, tip = 11) {
    return scene.route({id: key('route'), points: points.map(([x, y]) => p(x, y)), stroke: color, width: 2.5, tip, layer: 7});
  }
  ...
  if (id === 'begin') {
    ...
    const tasks = trace.tasks.slice(1).map((task, i) => node(`SwShapeTask · ${task.id}`, `${task.spans.length} spans · opacity ${task.opacity}`,
      190 + i * 580, 75, 'control', 300));
    const loop = fields('for (paint : paints)  →  renderShape()', 480, 158, 21);
    const offscreen = buffer('Offscreen', blank, 400, 330, 190);
    buffer('Canvas Surface', trace.background, 790, 330, 150, 'BG unchanged');
    const target = fields('surface = Offscreen', 400, 465, 21);
    const counters = [];
    const state = [...blank];
    beat('A and B have prepared RLE spans. The selected render target is Offscreen.');
    for (let i = 0; i < trace.writes.length; i++) {
      select(tasks[i], tasks[i - 1]);
      const link = route(i === 0 ? [[190, 134], [190, 330], [297, 330]] : [[770, 134], [770, 190], [600, 190], [600, 330], [503, 330]], i ? C.blue : C.orange);
      show(link, .2);
      for (const frame of replaySpans(trace.writes[i], state)) {
        replace(offscreen, frame.pixels, .24);
        if (counters.length) hide(counters.at(-1), .001);
        const counter = fields(`${trace.tasks[i + 1].id}  ${frame.completed} / ${trace.writes[i].length} spans`, 400, 505, 19);
        counters.push(counter); show(counter, .001);
        state.splice(0, state.length, ...frame.pixels);
      }
      beat(`${trace.tasks[i + 1].id} span writes accumulate in the same Offscreen.`);
    }
    if (counters.length) hide(counters.at(-1), .001);
    show(mark(offscreen));
    show(fields(`pixel (${trace.point.join(', ')}) · RGBA ${rgba(trace.offscreen[pointIndex]).join(' / ')}`, 400, 505, 19));
    beat('B is drawn after A. The resulting premultiplied image becomes the effect input.', 2.5);
    void loop; void target;
  } else if (id === 'chain') {
    ...
    const input = buffer('Fill output', trace.compositionInput, 140, 195, 150);
    const parent = buffer('Canvas BG', trace.background, 365, 195, 150);
    fields(`pixel (${trace.point.join(', ')}) · premultiplied RGBA`, 252, 35, 20);
    mark(input); mark(parent);
    fields(rgba(trace.compositionInput[pointIndex]).join(' / '), 140, 310, 18);
    fields(rgba(trace.background[pointIndex]).join(' / '), 365, 310, 18);
    const terms = compositeTerms(trace.compositionInput[pointIndex], trace.background[pointIndex], trace.opacity);
    const groups = [group(), group(), group()];
    const labels = ['Source term', '+ Canvas term', '= Output pixel'];
    const values = [terms.foreground, terms.background, terms.result];
    [105, 185, 265].forEach((y, i) => {
      text(groups[i], labels[i], 690, y, 20);
      text(groups[i], values[i].join(' / '), 690, y + 31, 18);
    });
    const target = fields('surface = recoverSfc', 252, 365, 20);
    const opacity = fields(`opacity ${trace.opacity} · ALPHA_BLEND`, 600, 365, 18);
    const composite = node('rasterDirectImage()', 'writes into Canvas', 520, 455, 'pixel', 300);
    const output = buffer('Final Canvas', trace.background, 835, 435, 120);
    const link = route([[675, 455], [763, 455]], C.pixel);
    beat('The effect input and existing Canvas refer to the same pixel coordinate.');
    show(target); beat('endComposite restores the Canvas render target without changing its pixels.');
    show(opacity); show(groups[0]); beat('Group opacity scales the premultiplied source with the engine integer operation.');
    show(groups[1]); show(groups[2]); beat('The remaining Canvas contribution is added to the scaled source.');
    show(output.g); show(link); select(composite); paintRows(output, trace.background, trace.final);
    beat('rasterDirectImage writes the native final pixels into the original Canvas buffer.', 2.5);
  } else if (id === 'output') {
