...
export function buildCurves({still=false,example=curveExample()}={}) {
  ...
  text(scene,'1. One control point → Two control points',50,120,25);
  ...
  text(result,'Same curve',455,565,20,blue,[.5,.5]);

  line(scene,[50,604],[1230,604],'#cccccc');
  text(scene,'2. Consecutive off-curve points → Implicit midpoint',50,646,25);
  ...
  text(middle,'Implicit on-curve point',855,790,21,muted);
  ...
  text(pairResult,'Same 2/3 rule for each segment',855,952,20,muted);
  const final=group('invariant');
  text(final,'Different control points. Same curve.',640,1040,25,ink,[.5,.5]);
