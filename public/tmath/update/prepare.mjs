...
function drawing(name) {
  ...
  const arrow = (parent, points, color = BLACK, dash = [6, 5]) => parent.route({
    ...
    fill: '#00000000', dash, tip: 9, layer: 8});
...
// Six beats: no prior data; initial flags; task allocation; request; results;
// Draw consumption and Sync retention. Same Paint/output slots as the next frame.
export function buildFirstFrame() {
  const model = firstFrameInputs(), d = drawing('first-frame');
  const {scene, group, text, rect, box, line, arrow, shape, show, hide, create, play, beat, finish} = d;
  text(scene, 'First frame · canvas->update()', 640, 44, 28);
  ['Scene', 'Paint', 'RenderData', 'Prepared'].forEach((label, i) => text(scene, label, [140, 405, 815, 1120][i], 113, 24, GRAY));
  box(scene, 'Scene', 140, 265, 170, 66);
  arrow(scene, [[225, 255], [278, 255], [278, 220], [345, 220]]);
  arrow(scene, [[225, 275], [278, 275], [278, 430], [345, 430]]);
  const inputs = group();
  text(inputs, 'Mparent × Mlocal', 151, 387, 21);
  text(inputs, `opacity = ${model.opacity}`, 151, 428, 21);
  text(inputs, 'clips = []', 151, 469, 21);
  text(inputs, 'parentFlag', 151, 510, 19, GRAY);
  text(inputs, model.parentFlag, 151, 546, 21);
  const flags = group(), allocated = group(), outputs = group(), routes = group();
  text(flags, 'parent | local', 590, 329, 22, GRAY);
  const empty = group(), resultBars = [];
  const lanes = model.paints.map((paint, i) => {
    const y = 220 + i * 210, color = i === 0 ? ORANGE : BLUE;
    shape(scene, paint.id, 402, y, color); text(scene, paint.id, 402, y + 72, 26, color);
    const flagBox = rect(flags, 590, y, 195, 94, WHITE, color);
    const parts = paint.effectiveFlag.split(' | ');
    text(flags, parts.slice(0, 2).join(' | '), 590, y - 17, 21, color, flagBox.key);
    text(flags, '| ' + parts[2], 590, y + 17, 21, color, flagBox.key);
    box(empty, 'rd = nullptr', 838, y, 268, 64, GRAY, 23);
    box(allocated, `SwShapeTask · ${paint.id}`, 838, y, 268, 64, BLACK, 23);
    text(allocated, 'new SwShapeTask', 838, y + 57, 19, GRAY);
    arrow(routes, [[690, y], [701, y]]);
    rect(scene, 1119, y + 7, 210, 126, '#00000000', color);
    text(scene, 'bounds + RLE', 1119, y + 104, 22, color);
    arrow(outputs, [[975, y], [1008, y]], color);
    // Type glyphs share the next frame's slots; these bars are not numeric RLE data.
    for (let row = 0; row < 5; row++) {
      resultBars.push(rect(outputs, i === 0 ? 1100 + row * 4 : 1119, y - 35 + row * 20,
        i === 0 ? 25 + row * 23 : row === 0 || row === 4 ? 112 : 145,
        11, color, '#00000000', 0).handle);
    }
    return {id: paint.id, color};
  });
  const worker = group();
  line(worker, [[60, 600], [1220, 600]], LIGHT);
  text(worker, 'prepare()', 214, 640, 23);
  text(worker, 'TaskScheduler::request()', 568, 640, 23);
  text(worker, 'SwShapeTask::run()', 980, 640, 23);
  box(worker, 'new task', 215, 715, 270, 70, BLACK, 22);
  arrow(worker, [[351, 715], [455, 715]]);
  rect(worker, 584, 715, 250, 90, '#00000000', BLACK);
  arrow(worker, [[710, 715], [803, 715]]);
  rect(worker, 991, 715, 370, 90, '#00000000', BLACK);
  const tokens = lanes.map((lane, i) => {
    const token = group();shape(token, lane.id, 583, 695 + i * 42, lane.color, .38);return token;
  });
  const draw = group(), synced = group();
  box(draw, 'Draw → done(A/B) → consume', 971, 834, 454, 57, BLACK, 21);
  text(synced, 'sync() · keep A/B rd', 277, 829, 22, GRAY);
  beat('New A and B have no prior RenderData; their prepared output slots are empty.', .8);
  show(inputs);show(flags);
  beat('Path and fill mark Path | Color; Scene insertion adds Transform, also inherited from the parent Scene.');
  hide(empty, .2);show(allocated);create(routes, .3);
  beat('prepare() allocates a new SwShapeTask for each null RenderData.');
  show(worker);tokens.forEach(token => show(token, .15));
  play(tokens.map(target => ({target, shift: [4.03, 0]})), .9);
  beat('Both task handles reach run(tid); their preparation may overlap.');
  show(outputs, .25);
  // Reveal the same symbolic result pieces that are already retained at next-frame start.
  resultBars.forEach(bar => create(bar, .045));
  beat('A and B each retain their first bounds and RLE in their own task.');
  show(draw);show(synced);
  beat('Draw consumes completed data; Sync retains both task objects for the next frame.', 2);
  return finish(model);
}

// Six beats: local mutation; inherited inputs; dispatch/reuse; request; worker commit;
// done() at consumption. The worker's duration/order is illustrative, not a trace of a race.
export function buildInvalidation(options) {
  const model = invalidationInputs(options), d = drawing('invalidation');
  const {scene, group, text, rect, box, line, arrow, shape, show, hide, create, play, move, beat, finish} = d;
  text(scene, `a->translate(${model.translation.join(', ')});  canvas->update();`, 640, 44, 28);
  ['Scene', 'Paint', 'RenderData', 'Prepared'].forEach((label, i) => text(scene, label, [140, 405, 815, 1120][i], 113, 24, GRAY));
  const tree = group();
  box(tree, 'Scene', 140, 265, 170, 66);
  arrow(tree, [[225, 255], [278, 255], [278, 220], [345, 220]]);
  arrow(tree, [[225, 275], [278, 275], [278, 430], [345, 430]]);
  const a = group(), b = group();
  shape(a, 'A', 402, 220, ORANGE); text(a, 'A', 402, 292, 26, ORANGE);
  shape(b, 'B', 402, 430, BLUE); text(b, 'B', 402, 502, 26, BLUE);
  const inputs = group();
  text(inputs, 'Mparent × Mlocal', 151, 387, 21);
  text(inputs, `opacity = ${model.opacity}`, 151, 428, 21);
  text(inputs, 'clips = []', 151, 469, 21);
  text(inputs, 'parentFlag = None', 151, 510, 21, GRAY);
  const flagA = group(), flagB = group();
  box(flagA, model.paints[0].effectiveFlag, 590, 220, 175, 60, ORANGE, 23);
  box(flagB, model.paints[1].effectiveFlag, 590, 430, 175, 60, GRAY, 23);
  const union = group();
  text(union, 'parent | local', 590, 329, 22, GRAY);
  const routeA = group(), routeB = group();
  arrow(routeA, [[678, 220], [701, 220]]);
  arrow(routeB, [[678, 430], [984, 430]], BLUE);
  text(routeB, 'skip() · reuse rd', 815, 387, 22, BLUE);
  const task = group();
  box(task, 'SwShapeTask · A', 838, 220, 268, 64, BLACK, 23);
  text(task, 'transform · flags · opacity', 838, 277, 19, GRAY);
  const outputA = group(), outputB = group();
  rect(outputA, 1119, 227, 210, 126, '#00000000', ORANGE);
  rect(outputB, 1119, 437, 210, 126, '#00000000', BLUE);
  const outputSpansA = [], outputSpansB = [];
  for (let row = 0; row < 5; row++) {
    outputSpansA.push(rect(outputA, 1100 + row * 4, 185 + row * 20, 25 + row * 23, 11, ORANGE, '#00000000', 0).handle);
    outputSpansB.push(rect(outputB, 1119, 395 + row * 20, row === 0 || row === 4 ? 112 : 145, 11, BLUE, '#00000000', 0).handle);
  }
  text(outputA, 'bounds + RLE', 1119, 324, 22, ORANGE);
  text(outputB, 'same bounds + RLE', 1119, 534, 22, BLUE);
  const worker = group();
  line(worker, [[60, 600], [1220, 600]], LIGHT);
  text(worker, 'prepare()', 214, 640, 23);
  text(worker, 'TaskScheduler::request()', 568, 640, 23);
  text(worker, 'SwShapeTask::run()', 980, 640, 23);
  box(worker, 'previous · done()', 215, 715, 270, 70, BLACK, 22);
  arrow(worker, [[351, 715], [455, 715]]);
  rect(worker, 584, 715, 250, 90, '#00000000', BLACK);
  arrow(worker, [[710, 715], [803, 715]]);
  rect(worker, 991, 715, 370, 90, '#00000000', BLACK);
  const token = group(); shape(token, 'A', 583, 715, ORANGE, .58);
  const commit = group();
  arrow(commit, [[1177, 714], [1242, 714], [1242, 225], [1224, 225]], ORANGE);
  const gate = group();
  box(gate, 'Draw → done(A) → consume', 971, 834, 414, 57, BLACK, 22);
  const returned = group();
  text(returned, 'postUpdate() → return', 277, 829, 22, GRAY);
  const changed = group();
  text(changed, 'Transform', 404, 341, 22, ORANGE);
  // The glyph is a Paint identity; its translation is derived from the API input.
  beat('A and B already retain prepared RenderData from the preceding frame.', .8);
  show(changed); move(a, model.translation[0] * 8, model.translation[1] * 8, .85);
  beat('The API marks only A with Transform. B has no local change.');
  show(inputs); show(union); show(flagA); show(flagB);
  beat('Paint combines the inherited matrix, opacity, clips and parent/local flags.');
  create(routeA); show(task); create(routeB);
  beat('A enters prepare(); B returns the same RenderData through skip().');
  show(worker); show(token);
  // Outline/spans in this scheduling view are type glyphs, not numerical RLE evidence.
  play(outputSpansA.map(target => ({target, opacity: .18})), .35);
  move(token, 403, 0, 1.05); show(returned);
  beat('Request schedules A; postUpdate can return while preparation is pending.');
  create(commit, .7);
  play(outputSpansA.map(target => ({target, opacity: 1, shift: [model.translation[0] * .03, -model.translation[1] * .03]})), .6, .08);
  hide(token); hide(changed);
  beat('The task commits new bounds and RLE to the existing A RenderData.');
  show(gate);
  beat('The consuming Draw path calls done() before reading A. B keeps its prior prepared data.', 2.3);
  return finish(model);
}
